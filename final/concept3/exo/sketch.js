...
function drawEndSequence() {
  ...
  textSize(32);
  textStyle(BOLD);
  text("EXO", width / 2, height / 2 - 300);
  image(img, width/2-125, height/2-280, 250, 250);
  textStyle(NORMAL);
  text("by Magdalena Bay", width / 2, height / 2 - 10);
...
function generateNotes() {
  ...
  if (currentTime - lastNoteTime >= 200) {
    ...
    switch (direction) { // create a new note in the chosen direction
      case 0: // left
        if (bass > 255) { // adjust the threshold for the left arrow
      ...
      case 1: // up
        ...
        break;
      case 2: // down
        if (lowMid > 180 && treble > 180) {
          notes.push(new Arrow("DOWN"));
        }
        break;
      case 3: // right
        if (highMid > 180 && bass > 160) {
