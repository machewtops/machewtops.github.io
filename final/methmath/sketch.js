...
function drawEndSequence() {
  ...
  textSize(32);
  textStyle(BOLD);
  text("Perreando Y Llorando", width / 2, height / 2 - 300);
  image(img, width/2-125, height/2-280, 250, 250);
  textStyle(NORMAL);
  text("by Meth Math", width / 2, height / 2 - 10);
...
function generateNotes() {
  ...
  if (currentTime - lastNoteTime >= 200) {
    ...
    switch (direction) { // create a new note in the chosen direction
      case 0: // left
        if (bass > 205) { // adjust the threshold for the left arrow
          notes.push(new Arrow("LEFT"));
        }
        break;
      case 1: // up
        if (mid > 240) { // adjust the threshold for the up arrow
          notes.push(new Arrow("UP"));
        }
        break;
      case 2: // down
        if (lowMid > 160 && treble > 150) {
