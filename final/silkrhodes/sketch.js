...
function drawEndSequence() {
  ...
  textSize(32);
  textStyle(BOLD);
  text("This Painted World", width / 2, height / 2 - 300);
  image(img, width/2-125, height/2-280, 250, 250);
  textStyle(NORMAL);
  text("by Silk Rhodes", width / 2, height / 2 - 10);
  ...
  if (currentTime - lastNoteTime >= 200) {
    ...
    switch (direction) { // create a new note in the chosen direction
      case 0: // left
        ...
        break;
      case 1: // up
        if (mid > 260) { // adjust the threshold for the up arrow
          notes.push(new Arrow("UP"));
        }
        break;
      case 2: // down
        if (lowMid > 175 && treble > 175) {
          notes.push(new Arrow("DOWN"));
        }
        break;
      case 3: // right
        if (highMid > 170 && bass > 170) {
